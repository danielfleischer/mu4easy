...
Complete and working mu4e setup using =mbsync= and mu1.9+. Setup includes working with *Google*, *Apple*, *GMX* and *Proton* together with aliases. There are additional packages to improve the experience; the main contribution is the context-generating macro which lets you add and manage accounts with ease. For the additional packages, =melpa= is used. 
...
Or install with Melpa (coming soon); e.g.
#+begin_src elisp
(use-package mu4easy
  :ensure t
  :bind ("C-c u" . mu4e))
